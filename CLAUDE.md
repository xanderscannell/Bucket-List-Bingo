...
> A family-friendly web app where users create personalized 5x5 bingo cards with bucket list items, track completions with photos/notes, and view leaderboards and activity feeds across all players.
...
If you see `[PLACEHOLDER]` markers (like `[PROJECT_NAME]`, `[CURRENT_TASK]`, etc.) in this file or in `.context/` files, the framework hasn't been initialized for this project yet. Before doing anything else:
...
4. **Suggest a commit message** for the user to commit the context updates — never commit or push automatically

## Current Focus

**Phase**: Maintenance & Enhancement
**Working on**: Bug fixes and UX polish (timezone handling, activity feed, modals)
**Key constraint**: Keep it simple — vanilla JS frontend, SQLite backend, local network deployment
